...
function DeclarationsSection() {
  ...
  const { data: rows = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      api.get<DeclarationRow[]>("/management/declarations", { params: { month } }).then((r) => r.data),
  });
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, roles }: { id: number; roles: Array<{ role_type: string; declared_hours: number }> }) =>
  ...
  function openEdit(decl: DeclarationRow) {
    setEditRoles(decl.roles.map((r) => ({ role_type: r.roleType, declared_hours: String(parseFloat(r.declaredHours).toFixed(1)) })));
    setEditDecl(decl);
    setApiError(null);
  }
  ...
  function handleEditSubmit(e: React.FormEvent) {
    ...
    updateMut.mutate({
      id: editDecl.id,
      roles: editRoles.map((r) => ({ role_type: r.role_type, declared_hours: parseFloat(r.declared_hours) })),
    });
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <Button size="sm" onClick={() => setCreateOpen(true)}>+ Add Declaration</Button>
      </div>
              ...
              {expanded && (
                <CardContent className="p-0">
                  {decls.map((decl) => {
                    ...
                    const totalHours = decl.roles.reduce((s, r) => s + parseFloat(r.declaredHours), 0);
                    return (
                      <div key={decl.id} className="border-b last:border-b-0">
                        <div className="flex items-center justify-between gap-3 px-4 py-2 bg-muted/10 text-sm flex-wrap">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-muted-foreground">· {decl.squad.name} · {totalHours.toFixed(0)}h total</span>
                            <Badge
                              variant="outline"
                              className={
                                isConfirmed
                          ...
                          <div className="flex gap-1.5">
                            ...
                            {decl.status === "draft" && (
                              <Button
                                ...
                                onClick={() => { setConfirmId(decl.id); setApiError(null); }}
                              >
                                Confirm
                              </Button>
                            )}
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow>
                              ...
                              <TableHead className="text-right">Declared Hours</TableHead>
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                            {decl.roles.map((r) => (
      ...
      {/* Create modal */}
      <ManagementModal isOpen={createOpen} onClose={() => setCreateOpen(false)} title="New Declaration">
        <DeclarationForm
          squads={squads}
          onSuccess={() => {
            setCreateOpen(false);
      ...
      >
        <form onSubmit={handleEditSubmit} className="space-y-4">
          ...
          <div className="flex gap-2 justify-end pt-2">
            <Button type="button" variant="outline" onClick={() => setEditDecl(null)}>Cancel</Button>
      ...
      <Dialog open={confirmId !== null} onOpenChange={(v) => { if (!v) setConfirmId(null); }}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">
            Declaration will be marked as confirmed.
          </p>
          <DialogFooter>
