...
function ClientsSection() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, f }: { id: number; f: ClientFormState }) =>
  ...
  function openEdit(c: ClientRecord) {
    setForm({ name: c.name, region: c.region, currency: c.currency });
    setEditing(c);
    setModalMode("edit");
  }
  function closeModal() {
...
function SowsSection() {
  ...
  const createMutation = useMutation({
    mutationFn: (f: SowFormState) =>
      api
        .post("/management/statement-of-works", {
          name: f.name,
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, f }: { id: number; f: SowFormState }) =>
      api
        .patch(`/management/statement-of-works/${id}`, {
          name: f.name,
          start_date: f.start_date,
  ...
  function openEdit(s: SowRecord) {
    setForm({
      name: s.name,
      client_id: s.clientId.toString(),
      start_date: fmtDate(s.startDate),
      end_date: s.endDate ? fmtDate(s.endDate) : "",
    });
    setEditing(s);
    setApiError(null);
    setModalMode("edit");
  ...
  return (
    <div className="space-y-4">
      ...
      <ManagementModal
        isOpen={modalMode !== null}
        onClose={closeModal}
        title={modalMode === "create" ? "Add Statement of Work" : `Edit SOW — ${editing?.name}`}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            {modalMode === "edit" && (
              <p className="text-xs text-muted-foreground">Client is immutable after creation.</p>
            )}
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
...
function ContractsSection() {
  ...
  const createMutation = useMutation({
    mutationFn: (f: ContractFormState) =>
      api
        .post("/management/contracts", {
          name: f.name,
          sow_id: Number(f.sow_id),
          hour_type: f.hour_type,
          type: f.type,
          assigned_hours: parseFloat(f.assigned_hours),
          start_date: f.start_date,
          end_date: f.end_date || undefined,
          status: f.status,
        })
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, f }: { id: number; f: ContractFormState }) =>
      api
        .patch(`/management/contracts/${id}`, {
          name: f.name,
          hour_type: f.hour_type,
          type: f.type,
          assigned_hours: parseFloat(f.assigned_hours),
  ...
  function openEdit(c: ContractRecord) {
    setForm({
      name: c.name,
      sow_id: c.sowId.toString(),
      hour_type: c.hourType,
      type: c.type,
      assigned_hours: c.assignedHours,
  ...
  return (
    <div className="space-y-4">
                  ...
                  {contracts.map((c) => (
                    <TableRow key={c.id} className={cn(c.status === "closed" && "opacity-60")}>
                      ...
                      <TableCell className="text-xs font-mono">
                        {c.type.replace("_", " ")} / {c.hourType}
                      </TableCell>
                      <TableCell className="text-sm text-right">
                        {parseFloat(c.assignedHours).toFixed(0)}
                      </TableCell>
                      <TableCell className="text-sm text-muted-foreground">
                        {fmtDate(c.startDate)}
                      </TableCell>
      ...
      <ManagementModal
        isOpen={modalMode !== null}
        onClose={closeModal}
        title={modalMode === "create" ? "Add Contract" : `Edit Contract — ${editing?.name}`}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          {apiError && <p className="text-sm text-destructive">{apiError}</p>}
          <div className="space-y-1.5">
          ...
          {modalMode === "create" && (
            <div className="space-y-1.5">
              ...
              <Select
                value={form.client_filter || "all"}
                onValueChange={(v) =>
                  setForm({ ...form, client_filter: v === "all" ? "" : v, sow_id: "" })
                }
              >
                <SelectTrigger>
          ...
          <div className="space-y-1.5">
            ...
            <Select
              required
              value={form.sow_id || "none"}
              onValueChange={(v) => setForm({ ...form, sow_id: v === "none" ? "" : v })}
              disabled={modalMode === "edit"}
            >
              <SelectTrigger>
            ...
            {modalMode === "edit" && (
              <p className="text-xs text-muted-foreground">SOW is immutable after creation.</p>
            )}
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              value={form.status}
              onValueChange={(v) =>
                setForm({ ...form, status: v as "active" | "paused" | "closed" })
              }
            >
              <SelectTrigger>
                <SelectValue />
