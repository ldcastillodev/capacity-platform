...
import { squadService, hourRecordService } from "@/lib/db";
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const row = await squadService.updateSquadMembership(Number(id), {
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  try {
    const row = await squadService.findMembershipById(Number(id));
    if (!row) {
    ...
    const dependentHours = await hourRecordService.countHoursByPersonSquadWindow(
      row.personId,
      row.squadId,
      row.effectiveFrom,
      row.effectiveTo
    ...
    const updated = await squadService.endDateMembership(row.id, endDate);
