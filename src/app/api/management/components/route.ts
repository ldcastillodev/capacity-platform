...
export async function GET(req: NextRequest) {
  ...
  const mappings = await componentMappingService.listManagedComponentMappings({
    includeArchived,
    today,
  });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const mapping = await componentMappingService.createManagedMappingWithOverlapResolution({
